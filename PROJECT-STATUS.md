...
- Sprint 0 · Task 3: `sources.yaml` (six domains' arXiv categories + ten official feeds), the `Fetcher` protocol, an arXiv fetcher honouring the 3-second rate limit, `radar ingest --since 1d`, `fetch_run` accounting, and a test proving a re-run inserts zero duplicates. (~4 h)
- Target unchanged: real documents in the database from a scheduled GitHub Actions run by 2026-09-29.
